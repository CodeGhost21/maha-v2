    ...
    if (commandName === "profile") {
      ...
      await User.findOne({ userID: interaction.user.id }).then(async (user) => {
        ...
        const discordMsgEmbed = new MessageEmbed()
          ...
          .setAuthor({ name: 'Gift of Eden', iconURL: 'https://i.imgur.com/xYG5x9G.png', url: 'https://peopleofeden.com/' })
          .setTitle('Title here')
      ...
      taskCollector?.on("collect", async (collected: any) => {
        ...
        await collected.reply({ content: `Hey ${collected?.user}, ${msg}`, ephemeral: true })
      })
      ...
      User.find({})
        ...
        .then((users) => {
          const top = users
            ...
            .slice(0, 10)
            .map(
              (u, i) => `${total_icons[i]} **${u.userTag}** - **${u.totalPoints}** points!`
            )
            .join("\n");
