...
import {
  updateMantaStakersData,
  updateMantaStakersAccumulate,
  updateMantaStakersBifrost,
} from "../controller/quests/stakeManta";
...
export const updateMantaPoints = async () => {
  ...
  do {
    ...
    const mantaUsersDataAccumulate = await updateMantaStakersAccumulate(
      addresses
    );

    for (const user of batch) {
      //manta
      const filteredMantaData = mantaUsersData.allMantaData.filter(
        (item: any) =>
          item.pacificAddress.toLowerCase().trim() ===
          user.walletAddress.toLowerCase().trim()
      );
      const mantaAmount = filteredMantaData.reduce(
        (accumulator: any, currentValue: any) =>
          accumulator + currentValue.stakingAmount,
        0
      );

      //accumulate manta
      const mantaBifrostAmount =
        mantaUsersDataBifrost.filter(
          (item) =>
      ...
      //bifrost manta
      const mantaAccumulateAmount =
        mantaUsersDataAccumulate.filter(
          (item) =>
            item.address.toLowerCase().trim() ===
            user.walletAddress.toLowerCase().trim()
        )[0]?.balance || 0;
      ...
      //total Manta points
      const totalMantaAmount =
        (mantaAmount + mantaBifrostAmount + mantaAccumulateAmount) *
        stakePtsPerManta;
