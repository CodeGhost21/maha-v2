...
const networks = [
    ...
    //     zeroTokenAddress: "0x861af65b499ac38fc767547fed9c44bba8515a4f",
    ...
    // },
    {
        ...
        zeroTokenAddress: "0x35a57eFB9b4ae833e9A200bb191ff69420caFa1D",
    ...
    //     zeroTokenAddress: "0x357f93E17FdabEcd3fEFc488a2d27dff8065d00f",
...
export const getData = async () => {
    ...
    let count = 0
    const queryURL = 'https://api.goldsky.com/api/public/project_clsk1wzatdsls01wchl2e4n0y/subgraphs/zerolend-manta/1.0.0/gn'
    do {
        ...
        console.log("executed blast-points for batch. last address", lastAddress, count)
...
export const main = async () => {
    console.log(networks[0].name);
    const first = 100;
    let batch: any;
    let lastAddress = "0x0000000000000000000000000000000000000000";
    let count = 0
    const queryURL = 'https://api.goldsky.com/api/public/project_clsk1wzatdsls01wchl2e4n0y/subgraphs/zerolend-blast/1.0.0/gn'
    do {
        const graphQuery = `query {
        users(where: {id_gt: "${lastAddress}"}, first: ${first}) {
          id
        ...
        try {
            batch = await axios.post(queryURL, { query: graphQuery }, { headers });
            count += batch.data.data.users.length
            const walletUserAddresses: string[] = batch.data.data.users.map(
                (u: any) => u.id
            ) as string[];
            const earlyZero = await getEarlyZeroBalance(
                networks[0].zeroTokenAddress,
                networks[0].incentiveControllerAddress,
                networks[0].aTokens,
                networks[0].variableDebtTokens,
                walletUserAddresses,
                networks[0].name,
                providers[`${networks[0].name}`]
            );
...
const writeCsvFile = (data: any) => {
    ...
    try {
        ...
        data.forEach((entry: any) => {
            const csvRow = `${entry.who},${entry.chain},${entry.claimedRewards ? entry.claimedRewards : 0
